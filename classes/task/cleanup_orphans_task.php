...
/**
 * Adhoc task that reviews the courses with imported versions and deletes
 * the orphan old-resourceid folders under editions/ whose versions are
 * already present in the current resourceid folders.
 *
 * Custom data: object with ->courseids (int[]), ->notfound (string[]),
 * ->dryrun (bool), ->scopelabel (string) and ->scopedesc (string).
 *
...
class cleanup_orphans_task extends adhoc_task {
    ...
    public function get_name(): string {
        return get_string('cleanuporphanstask', 'local_educaaragon');
    }

    /**
     * Reviews each course of the custom data list and writes the log document.
     *
     * @return void
     * @throws coding_exception
    ...
    public function execute(): void {
        ...
        $log('============================================================');
        $log(' LIMPIEZA DE CARPETAS HUERFANAS DE EDICIONES (tarea en segundo plano)');
        ...
        foreach ($courseids as $courseid) {
            ...
            ob_start();
            try {
                $migrator = new edition_versions_migrator($repository, $dryrun, '', false, true);
                $coursestats = $migrator->cleanup_course_orphans($course);
        ...
        $log('');
        $log(get_string('cleanup_result', 'local_educaaragon', (object)[
        ...
        mtrace(PHP_EOL . get_string('cleanuporphanstask', 'local_educaaragon') . PHP_EOL . implode(PHP_EOL, $summary));

        try {
            write_execution_log('limpieza_' . $scopelabel . ($dryrun ? '_dryrun' : ''), $loglines);
        } catch (Exception $e) {
            mtrace($e->getMessage());
        }
    }
